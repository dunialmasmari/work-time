<?php

return [

    /*
    |--------------------------------------------------------------------------
    | Pagination Language Lines
    |--------------------------------------------------------------------------
    |
    | The following language lines are used by the paginator library to build
    | the simple pagination links. You are free to change them to anything
    | you want to customize your views to better match your application.
    |
    */

//[
  
    'Footer'=> [
      'about_us'=> 'من نحن',
      'contact_us'=> 'تواصل معنا',
      'allrights'=> 'جميع الحقوق محفوظه لموقع وورك تايم-2021',
      'subscribemassage'=>'الإشتراك في التنبيهات الإلكترونية للمناقصات والوظائف',
    ],
    'Header'=> [
      'language'=> [
        'ar'=> 'العربية',
        'en'=> 'الانجليزية'
      ],
      'Menu'=> [
        'accountSetting'=> 'إعدادات الحساب',
        'logOut'=> 'تسجيل الخروج',
        'account'=> 'الحساب الشخصي'
      ]
    ],
    'Navbar'=> [
      'Home'=> 'الرئيسية',
      'tenders'=> 'المناقصات',
      'about_us'=> 'من نحن',
      'contact_us'=> 'تواصل معنا',
      'en'=> 'English',
      'ar'=> 'عربي',
      'login'=> 'تسجيل الدخول',
      'signup'=> 'انشاء حساب',
      'service' =>'الخدمات',
      'blog'=> 'المدونات ',
    ],
    'aboutus'=> [
      'description'=>'وورك تايم هو من المواقع الإلكترونية المتخصصة  في إعلان ونشر جميع المناقصات الأنشائية والتوريدات.. وإعلانات التأهيل المسبق للمقاولين والموردين ومقدمي الخدمات وكذالك هو صلة الوصل بين الباحثين عن عمل وأصحاب العمل الذين ينوون التوظيف.

      تم اطلاق موقع وورك تايم عام 2021، حيث انه يساعد أصحاب العمل، الشركات، وألمنظمات الغير ربحية في اليمن بإضافة العديد من الوظائف الشاغرة على منصة وورك تايم للوصول الى اكبر شريحة ممكنه وسهولة متابعة المناقصات والوظائف الجديده..',
      'vesion'=>'رؤيتنا',
      'vesionDes'=>'وورك تايم تتطمح الى ان تكون الشركة الأكثر تميزا في تقديم افضل الخدمات والعروض المنافسة لكل الافراد والمؤسسات بشكل اسهل واسرع..',
      // 'services'=>'خدماتنا',
      // 'servicesDes'=>'تزويد الباحثين عن العطاءات والمناقصات الجديده في كل المجالات من خلال موقعنا بكل سهوله وسرعه وبميزه وصولهم تنبيه اشعارات بكل المناقصات الجديده...  وكذالك تزويد الباحث عن عمل بالقدره علئ العثور علئ الوظيفه التي يرئ نفسه مؤهل لها من خلال ادوات البحث عن جميع الوظائف المصنفه في موقعنا وبخدمه شعارات التنبيهات للوظائف والاشتراك بالفرص الجديده ..'
    ],
    'Drawer'=> [
      'Dashboard'=> 'الرئيسية',
      'jobs&tendersManagement'=>'إدارة الوظائف و المناقصات ',
      'tendersManagement'=> 'إدارة المناقصات',
      'MajorsManagement'=> 'إدارة الاقسام'
    ]
  ,
  'validation'=> [
    'startDateValidation'=> 'يجب ان يكون تاريخ نشر المناقصه اصغر من تاريخ الانتهاء',
    'endDateValidation'=> 'يجب ان يكون تاريخ الانتهاء  اكبر من تاريخ النشر ',
    'filetype'=> 'يجب ان تكون صيغة الملف zip او pdf',
    'emptyfieldrequired'=> 'يجب ملىء هذا الحقل',
    'chooserequired'=> 'هذا الحقل ضروري',
    'exceededlimitation'=> 'لقد تجاوزت الحد المسموح للكتابه',
    'undermainlimitation'=> 'يجب ان لا يكون اقل من 50 حرف ',
    'emailvalidation'=> 'يجب كتابة الايميل بشكل صحيح',
    'wrongfieldinfo'=> 'خطاء في ادخال البيانات',
    'passwordnotmuch'=> 'كلمة المرور لا تتطابق',
    'phoneno'=> 'يجب ملئ هذا الحقل بالبيانات وبشكل صحيح',
    'imagename'=> 'الملف الذي اخترته ليس صوره',
    'imagesize'=> 'حجم الملف يجب ان يكون 500*500',
    'requredEnglish'  =>'يمكنك تنزيل سيرتك الذاتية باللغة الانجليزية فقط ',
    'imagetype'=> 'الصوره يجب ان تكون من نوع png ,jpg,jpeg',
    'PasswordValidation'=>'يجب ان تكون اكثر من 8 وتحتوي على احرف صغيره،احرف كبيره وارقام ورمز واحد على الاقل ',
    'example'=> [
      'name'=> 'مثال=> محمد احمد',
      'firstname'=> 'مثال=> محمد',
      'lastname'=> 'مثال=>احمد',
      'email'=> 'مثال=>example@gmail.com'
    ]
  ],
  'Home'=> [
    'Tenders'=> 'المناقصات',
    'latestTenders'=> 'اخر المناقصات',
    'majorsTenders'=> 'المناقصات على حسب الاقسام',
    'Tenders&JobsAlerts'=> 'إشعارات ',
    'visti_website' =>' زيارة الموقع ',
  ],
  'LogIn'=> [
    'LogInTitle'=> 'تسجيل الدخول ',
    'ForgitPwd'=> 'نسيت كلمة المرور',
    'RemmberMe'=> 'تذكرني',
    'Submit'=> 'دخول',
    'Email'=> 'البريد الالكتروني',
    'Pwd'=> 'كلمة المرور',
    'IhaveAccount'=> 'ليس لدي حساب'
  ],
  'Tenders'=> [
    'Title'=> 'المناقصات',
    'name'=> 'عنوان المناقصه',
    'image'=> 'صورة المناقصه',
    'downloadpdfs'=> 'تحميل الملفات',
    'major'=> 'القسم',
    'company'=> 'الجهة',
    'more'=> 'مزيد من المعلومات',
    'location'=> 'المنطقة', 
    'applyLink'=> 'رابط التقديم',
    'startDate'=> 'تاريخ البدء',
    'postedDate'=> 'تاريخ النشر',
    'Deadline'=> 'تاريخ الانتهاء',
    'description'=> 'وصف المناقصة',
    'save'=> 'حفظ',
    'action'=> 'الحدث',
    'AddNew'=> 'إضافة مناقصة جديد',
    'filter'=> 'فرز'
  ],
  'Majors'=> [
    'Title'=> 'الأقسام',
    'Tenders'=> 'أقسام المناقصات',
    'Jobs'=> 'أقسام الوظايف',
    'AddNew'=> 'إضافة قسم جديد',
    'UpdateMajor'=> 'تعديل القسم',
    'name'=> 'عنوان القسم',
    'active'=> 'مفعل',
    'disabled'=> 'غير مفعل',
    'activate'=> 'تفعيل',
    'deactivate'=> 'إلغاء التفعيل',
    'status'=> 'الحالة',
    'save'=> 'حفظ',
    'Update'=> 'تعديل',
    'Delete'=> 'حذف',
    'action'=> 'الحدث'
  ],
  'apisuccessmesages'=> [
    'title'=> 'تم بنجاح'
  ],
  'apifailmessages'=> [
    'apiFail'=> 'حدث خطاء ما تحقق من اتصالك بالانترنت ،ان استمرت المشكله تواصل مع مركز الدعم لمعرفة السبب',
    'title'=> 'عذرا'
  ],
  'ContactUS'=> [
    'Tittle'=>'تواصل معنا',
    'Name'=> 'إسمك',
    'Email'=> 'بريدك الالكتروني',
    'Send'=> 'أرسل',
    'Message'=> 'ضع رسالتك هنا',
    'description'=> 'إذا كانت لديك أي أسئلة بخصوص خدماتنا ، يرجى ملء النموذج أدناه وسنحاول الرد عليك في أقرب وقت ممكن. '
  ],
  'cities'=> [
    'Abyan'=> 'أبين',
    'Aden'=> 'عدن',
    'AlMahrah'=> 'المهرة',
    'AlMahwit'=> 'المحويت',
    'Sanaa'=> 'صنعاء',
    'Amran'=> 'عمران',
    'Dhale'=> 'الضالع',
    'Dhamar'=> 'ذمار',
    'Hadramaut'=> 'حضرموت',
    'AlJawf'=> 'الجوف',
    'Hajjah'=> 'حجة',
    'Ibb'=> 'إب',
    'Lahij'=> 'لحج',
    'Marib'=> 'مأرب',
    'AlBayda'=> 'البيضاء',
    'Raymah'=> 'ريمة',
    'Sadah'=> 'صعدة',
    'AmanatAlAsimah'=> 'أمانة العاصمة',
    'Shabwah'=> 'شبوة',
    'Socotra'=> 'سقطرى',
    'Taiz'=> 'تعز',
    'AlHodaida'=>'الحديدة',
    'Yemen'=>'اليمن',
  ],

  'Jobs'=> [
    'Title'=>'الوظائف',
    'Titles'=>'قائمة الوظائف',
    'name'=> 'عنوان الوظيفة',
    'image'=> 'صورة الوظيفة',
    'downloadpdfs'=> 'تحميل الملفات',
    'major'=> 'القسم',
    'more'=> 'مزيد من المعلومات',
    'location'=> 'المنطقة',
    'company'=> 'الشركة/المنظمة',
    'applyLink'=> 'رابط التقديم',
    'startDate'=> 'تاريخ البدء',
    'postedDate'=> 'تاريخ النشر',
    'Deadline'=> 'تاريخ الانتهاء',
    'description'=> 'وصف الوظيفة',
    'save'=> 'حفظ',
    'action'=> 'الحدث',
    'AddNew'=> 'إضافة وظيفة جديدة',
    'filter'=> 'فرز حسب',
    'Notify'=> 'انشاء تنبية وظيفي',
    'cv' => 'ادخال سيرتك الذاتية هنا ',
    'RCom' => 'التوصيات التي حصلت عليها',
    'req_pdf'=>'يجب ان يكون pdf',
    'submit'=>'التقديم الان ',
    'applaying'=>' للتقديم عبر موقعنا يرجى ادخال بياناتك',
    'others' => 'مزيد من الوظائف ',
    'applaylink'=> 'لزيارة موقع التقديم  ',
    'applayEmail'=> 'لمراسلة ايميل التقديم ',
    'toapplying'=>'لزيارة رابط او ايميل التقديم  ',
    'Active'=> 'مفعل',
    'notActive'=> 'غير مفعل',
    'userAdd'=>'اضيف بواسطة',

  ],

  'Services'=> [
    'Title'=>' الخدمات ',
    'Titles'=>' قائمة الخدمات ',
    'others'=> 'خدمات اخرى ',
    'description' => 'وصف الخدمة ',
  ],
  'blogs'=> [
    'Titles'=> 'المدونات',
    'Title'=> 'المدونة',
    'others'=> 'مقالات  اخرى ',
    'description' => 'وصف المقال ',

  ],

   // Dashbored

  /** Advertisment */
  /**Show Advertising */
  'Advertising'=>[
    'title'=>'إعلانات',
    'Title'=> 'قائمة لإعلانات',
    'Title'=> 'عنوان الإعلان ',
    'image'=> 'الصورة',
    'AdvertisingPosition' => ' مكان عرض الإعلان',
    'link'=> 'الرابط',
    'status'=> 'الحالة',
    'Actions'=> 'الحدث',
    'Active'=> 'مفعل',
    'notActive'=> 'غير مفعل',
  ],
  /**Add Advertising*/
  'AdvertisingAdd'=>[
    'TitlePage'=> 'اضافة اعلان جديد',
    'image'=>'صورة الإعلان',
    'choose'=> 'اختار صورة', 
    'Home'=> 'الصفحة الرئيسية',
    'Siderbar'=> 'بجانب الصفحات',
    'Title'=> 'عنوان الاعلان',
    'link'=> 'الرابط',
    'Submit'=> 'اضافة الإعلان',
  ],
  /**Edite Advertising*/
  'AdvertisingEdite'=>[
    'Tilte'=> 'تعديل الإعلان',
    'Submit'=> 'تعديل الإعلان ',
  ],

  /** Blogs */
  /**Show Blogs */
  'Blog'=>[
    'TitlePage'=> 'قائمة المدونات',
    'Title'=> 'عنوان المدونة',
    'SubTiltle'=> 'العنوان الفرعي',
    'image'=> 'الصورة',
    'description'=> 'الوصف',
    'status'=> 'الحالة',
    'Actions'=> 'الحدث',
    'Active'=> 'مفعل',
    'notActive'=> 'غير مفعل',
  ],
  /**Add Blogs */
  'BlogAdd'=>[
    'TitlePage'=> 'اضافة مدونة جديدة',
    'image'=> 'صورة المدونة',
    'choose'=> 'اختار صورة',
    'Title'=> 'عنوان المدونة',
    'SubTiltle'=> 'العنوان الفرعي',
    'description'=> 'وصف المدونة',
    'Submit'=> 'اضافة المدونة'
  ],
  /**Edite Blogs */
  'BlogEdite'=>[
    'Title'=> 'تعديل المدونة',
    'Submit'=> 'تعيل المدونة',
  ],

  /** Jobs */
  /**Show Jobs */
  'Jobsshow'=>[
    'TitlePage'=> 'قائمة الوظائف',
    'Title'=> 'عنوان الوظيفة',
    'Location'=> 'الموقع',
    'Company'=> 'الشركة',
    'deadline'=> 'تاريخ اٌنتهاء ',
    'status'=> 'الحالة',
    'Actions'=> 'الحدث',
    'Active'=> 'مفعل',
    'notActive'=> 'غير مفعل',
  ],
  /**Add Jobs */
  'JobsAdd'=>[
    'TitlePage'=> 'اضافة وظيفة جديدة',
    'Title'=> 'العنوان',
    'Major'=> 'القسم',
    'Location'=> 'الموقع',
    'OtharLocation'=> 'موقع اخر',
    'Company'=> 'الشركة',
    'start_date'=> 'تاريخ البدء',
    'image'=> 'صورة الوظيفة ',
    'choose'=>'اختار صورة',
    'deadline'=> 'تاريخ اٌنتهاء ',
    'PostedDate'=> 'تاريخ النشر',
    'Description'=> 'وصف الوظيفة',
    'massege'=> 'هل يكون التقديم للوظيفة بواسطة الموقع أو التواصل مع الشركة؟',
    'No'=> 'لا',
    'Yes'=> 'نعم',
    'Both'=> 'كلاهما',
    'link'=> 'الرابط',
    'email'=> 'البريد الالكتروني',
    'Recommendation'=> 'التوصية',
    'Submit'=> 'اضافة وظيفة',
  ],
  /**Edite Jobs */
  'JobsEdite'=>[
    'Title'=> 'تعديل الوظيفة',
    'Link'=> 'رابط التقديم',
    'Submit'=>'تعديل الوظيفة',
    'register'=> 'التسجيل عبر',
    'website'=> 'الموقع',
    'company'=> 'الشركة',
  ],

  /** Majors */
  /**Show Major */
  'Major'=>[
    'Title'=> 'قائمة الأقسام',
    'Name'=> 'اسم القسم ',
    'Type'=> 'النوع',
    'status'=> 'الحالة',
    'Actions'=> 'الحدث',
    'Active'=> 'مفعل',
    'notActive'=> 'غير مفعل',
  ],
  /**Add Major */
  'MajorAdd'=>[
    'Title'=> 'اضافة قسم جديد',
    'Name'=> 'اسم القسم ',
    'Type'=> 'النوع',
    'Tender'=> 'مناقصة',
    'Job'=> 'وظيفة',
    'Submit'=> 'اضافة قسم',
  ],
  /**Edite Major */
  'MajorEdite'=>[
    'Title'=> 'تعديل القسم',
    'Name'=> 'اسم القسم ',
    'Type'=> 'النوع',
    'Tender'=> 'مناقصة',
    'Job'=> 'وظيفة',
    'Submit'=> 'تعديل القسم',
  ],

  /** Services */
  /**Show Service */
  'Service'=>[
    'TitlePage'=> 'قائمة الخدمات',
    'Title'=> 'عنوان الخدمة',
    'image'=> 'الصورة',
    'description'=> 'الوصف',
    'status'=> 'الحالة',
    'Actions'=> 'الحدث',
    'Active'=> 'مفعل',
    'notActive'=> 'غير مفعل',
  ],
  /**Add Service */
  'ServiceAdd'=>[
    'TitlePage'=> 'إضافة خدمة جديدة',
    'image'=> 'صورة الخدمة ',
    'choose'=> 'اختار الصورة',
    'Title'=> 'عنوان الخدمة',
    'description'=> 'وصف الخدمة',
    'Submit'=> 'اضافة الخدمة',
  ],
  /** Edite Service */
  'ServiceEdite'=>[
    'Title'=> 'تعديل الخدمة',
    'Submit'=> 'تعديل الخدمة',
  ],

  /**Tender */
  /** Show Tender */
  'Tender'=>[
    'TitlePage'=> 'قائمة المناقصة ',
    'Title'=> 'عنوان المناقصة ',
    'location'=> 'الموقع',
    'company'=> 'الشركة',
    'deadline'=> 'تاريخ الإنتهاء',
    'status'=> 'الحالة',
    'Actions'=> 'الحدث',
    'Active'=> 'مفعل',
    'notActive'=> 'غير مفعل',
  ],
  /**Add Tender */
  'TenderAdd'=>[
    'choosemajor'=> 'اختر قسم',
    'chooseposition'=> 'اختر مكان الإعلان',
    'chooseCity'=> 'اختر مدينة',
    'TitlePage'=> 'إضافة مناقصة جديدة',
    'Title'=> 'عنوان المناقصة',
    'Major'=> 'القسم',
    'File'=> 'ملف المناقصة ',
    'chooseFile'=>'اختار الملف',
    'Location'=> 'الموقع',
    'OtharLocation'=> 'موقع اخر ',
    'Company'=> 'الشركة',
    'Link'=> 'رابط التقديم ',
    'start_date'=> 'تاريخ البدء',
    'image'=> 'صورة المناقصة ',
    'choose'=>'اختار صورة',
    'deadline'=> 'تاريخ الإنتهاء',
    'PostedDate'=> 'تاريخ النشر ',
    'Description'=> 'وصف المناقصة',
    'Submit'=> 'إضافة مناقصة',
  ],
  /**Validte Tender */
  'TenderValidate'=>[
    'requerMassage'=> '*يجب ملئ هذا الحقل.',
    'OptionMassage'=> '*هذا الحقل اختاري.',
    'fileMassage'=> '*يجب أن يكون الملف من نوع pdf,zip .',
    'locationMassage'=> '*يمكنك اختيار موقع واحد او اكثر .',
    'otharLocationMassage'=> '*عند كتابة أكثر من موقع يرجى كتابة فاصلة (,) بينهم.',
    'companyMassage'=> '* أرالنص المدخل يجب أن يكون حروف فقط ولايحتوي على رموز أو أرقام  ',
    'emailMassage'=> '*أدخل الإيميل بالصيغة الصحيحة',
   'phoneMassage'=> '*أدخل الرقم الصيغة الصحيحة',
    'linkMassage'=> '*أدخل الرابط بالصيغة الصحيحة',
       'nameMassage'=> '*يجب أن يكون أقل من 25 حرف ',
       'longText'=> '*يجب أن يكون النص اكبر من 50',
        'midText'=> '*يجب أن يكون اكبر من 10 احرف واقل من 50 حرف ',
    'imageMassage'=> '*طول وعرض الصورة يجب أن يكون مابين 200 و 500 بيكسل',
    'imageaderMassage' => 'طول الصورة يجب أن يكون مابين 200 و 600 بيكسل و*عرض الصورة يجب أن يكون مابين 600 و 900 بيكسل ',
    'deadlineMassage'=> '*تاريخ الإنتهاء يجب أن يكون أكبر من تاريخ البدء.',
    'PosteDateMassage'=> '* تاريخ النشر يجب أن يكون أصغر من تاريخ الإنتهاء وأكبر من تاريخ البدء.',
  ],
   /**Edite Tender */
   'TenderEdite'=>[
    'Title'=> 'تعديل المناقصة',
    'Submit'=> 'تعديل المناقصة',
  ],
   /** Users */
  /**Show Users */
  'Users'=>[
    'Title'=> ' قائمة المستخدمين',
    'Name'=> 'الإسم',
    'UserName'=> 'اسم الستخدم',
    'fullName'=> 'إسم المستخدم الكامل',
    'Email'=> 'البريد الالكتروني',
    'status'=> 'الحالة',
    'Actions'=> 'الحدث',
    'Active'=> 'مفعل',
    'notActive'=> 'غير مفعل',
    'applyAsCompany'=>"التسجيل ك شركة",
    'applyAsEmplyee'=>"الاشتراك في الموقع",
    'Login'=>'تسجيل الدخول',
    'dontHaveAccount'=>"لا تملك حسابا بعد؟",
    'HaveAccount'=>"لديك حساب؟",
    'or'=>'أو',
    'register'=>'تسجيل',
    'createUserAccount'=>'تسجيل حساب مشترك',
    'createEmployerAccount'=>'تسجيل حساب شركة '
  ],
   /**Add Users */
   'UserAdd'=>[
    'TitlePage'=> 'إضافة مستخدم جديد',
    'Password'=> 'كلمة المرور',
    'ConfirmPassword'=> 'تأكيد كلمة المرور',
    'Submit'=> 'إضافة مستخدم',
  ],
  /**Edite Users */
  'UserEdite'=>[
    'Title'=> 'تعديل المستخدم',
    'oldPassword'=> 'كلمة المرور القديمة',
    'NewPassword'=> 'كلمة المرور الجديدة',
    'ChangePassword'=> 'تغيير كلمة المرور',
    'Submit'=> 'تعديل المستخدم',
  ],
  'Sidebar'=>[
    'Dashboard'=>'لوحة التحكم',
    'Reports'=>'التقارير',
    'majorMang'=> 'إدراة الاقسام ',
    'majorsAll'=>'عرض كافة الاقسام ',
    'tenderMang'=>'إدارة المناقصات ',
    'tendersAll'=>'عرض كافة المناقصات ',
    'addTender'=> 'إضافة مناقصة جديدة ',
    'jobMang'=> 'إدارة الوظائف ',
    'jobAll'=> 'عرض كافة الوظائف ',
    'addJob'=> 'إضافة وظيفة جديدة ',
    'serviceMang'=> 'إدارة الخدمات ',
    'servAll'=> 'عرض كافة الخدمات ',
    'addServ'=> 'إضافة خدممة جديدة ',
    'blogMang'=>'إدارة المدونات  ',
    'blogAll'=> 'عرض كافة المدونات  ',
    'addBlog'=> 'اضافة مدونة جديدة ',
    'adverMang'=>'إدارة الإعلانات ',
    'adverAll'=> 'عرض كافة الإعلانات ',
    'addAdver'=> 'إضافةإعلان جديد  ', 
    'dashuserMang'=>' إدارة مستخدمي لوحة التحكم',
    'compuserMang'=>'ادارة الشركات',
    'seruserMang'=>'ادارة الباحثين',
    'userAll'=> 'عرض كافة المستخدمين',
    'adduser'=> 'إضافة مستخدم جديد',
    'Allcomp'=> 'عرض كافة الشركات',
    'addcomp'=> 'إضافة شركة جديدة',
    
    'logout' => 'تسجيل الخروج',
  ],
  'companyInfo'=>[
    'Title'=> 'قائمة الشركات',
    'intersted'=> 'مهتم ب ',
    'companyinfo'=> 'معلومات عن الشركة',
    'Changelogo'=> 'تغير شعار الجهة ',
    'logo'=> 'الشعار',
    'Submit'=> 'إضافة الشركة',
    'CompanyName' => 'الاسم  ',
    'CompanyNames' => 'اسم الجهة ',
    'WebsiteLink' => 'رابط الجهة ',
    'Email' => 'البريد الالكتروني ',
    'Emails' => 'البريد الالكتروني الخاص بالجهة ',
    'Phone'=>'الهاتف   ',
    'Phones'=>'هاتف الجهة ',
    'Country' => 'الدولة ',
    'City'=>'المدينة ',
    'Address'=>'العنوان ',
    'Industry'=>'الصناعة ',
    'Type'=>'النوع ',
    'Types'=>' نوع عمل الجهة',
    'Founded'=>'تاريخ التأسيس',
    'size'=>'حجم الجهة',
    'sizes'=>'حجم استيعاب الموظفين ',
    'About'=>'عن الجهة ',
    'AboutCompany'=> 'عن الشركة',
    'Description'=>'الوصف ',
    'SaveChanges'=>'حفظ التغيرات ', 
    'Descriptions'=>' اكتب وصف عن جهتكم واعمالكم لنتعرف عليكم اكثر',
    'Abouts'=>' اكتب نبذة صغيرة عن جهتكم ',
    'compinformation'=>'معلومات عن الجهة ',
    'tender'=>'نشر مناقصة   ',
    'job'=>'نشر وظيفة  ',
    'Applications'=>'عرض التطبيق ',
    'companydetails'=>'تفاصيل عن الشركة',
    'companyName'=>'اسم الشركة',
    'statucompany'=>'قيد الإنتظار',
    'jobRejrct'=>'وظائف ملغية  ',
    'jobAccept'=>'وظائف نشرت  ',
    'tenderRejrct'=>'مناقصات ملغية',
    'tenderAccept'=>'مناقصات نشرت ',
    ''=>'',
    ''=>'',
    ''=>'',
  ],
  'userInfo'=>[
    'userdetials'=> 'معلومات المستخدم',
    'Changepic'=>'تغير صورتك الشخصية',
    'Profile'=>'الملف الشخصي ',
    'Experience'=>'الخبرات ',
    'Education'=>'التعليم والتدريب ',
    'Projects'=>'المشاريع ',
    'References'=>'المراجع ',
    'Languages'=>'اللغات ',
    'Skills'=>'المهارات',
    'FullName'=>'الاسم الكامل ',
    'WebsiteLink'=>'رابط موقعك ',
    'Email'=>'البريد الالكتروني ',
    'Phone'=>'الهاتف ',
    'Country'=>'الدولة ',
    'City'=>'المدينة ',
    'Gender'=>'الجنس',
    'Status'=>'الحالة الاجتماعية ',
    'About'=>'نبذة عنك',
    'Abouts'=>'اكتب وصف بسيط عن نفسك ',
    'aboutUser'=> 'عن المستخدم',
    'SaveChanges'=>'حفظ التغيرات ',
    'From'=>'من',
    'update'=>'تعديل ',
    'addnew'=>'اضافةجديدة  ',
    'AddEducation'=>'اضافة تعليم ',
    'To'=>'الى ',
    'AddRefer'=>'اضافة مرجع ',
    'AddLang'=>' اضافة لغة ',
    'addSkill'=>'اضافة مهارة',
    'AddPro'=>'اضافة مشروع ',
    'Title'=>'العنوان ',
    'Subtitle'=>'العنوان الفرعي',
    'startdate'=>'تاريخ البدء  ',
    'enddate'=>'تاريخ الانتهاء',
    'description'=>'الوصف ',
    'DescriptionExperience'=>'اكتب وصف بسيط عن خبراتك ',
    'DescriptionEducation'=>'اكتب وصف بسيط عن التعليم والتدريب الذي تلقيته ',
    'DescriptionProjects'=>'اكتب وصف بسيط عن المشاريع التي قمت بها ',
    'DescriptionReferences'=>'اكتب وصف بسيط عن الاشخاص والمراجع لك ',
    'Save'=>'حفظ ',
    'Close'=>'اغلاق ',
    'AddExper'=>'اضافة خبرة ',
    'langName'=>' اسم اللغة ',
    'langLevel'=>'مستوى اللغة ',
    'Delete'=>'حذف ',
    'DeleteMass'=>'هل انت متاكد من عملية الحذف ؟',
    'DeleteSure'=>'تأكيد الحذف',
    'NoDelete'=>'الغاء الامر',
    'ReferPhone'=>'الهاتف ',
    'RefersPhone'=>'هاتف المرجع',
    'ReferName'=>'الاسم ',
    'ReferEmail'=>'الايميل ',
    'SkilName'=>'اسم المهارة  ',
    'SkilLevel'=>'مستوى المهارة ',
    'updateExperience'=>'تعديل الخبرات ',
    'updateEducation'=>'تعديل التعليم ',
    'updateProject'=>'تعديل المشروع  ',
    'updateReferences'=>'تعديل المرجع ',
    'updateLanguages'=>'تعديل اللغة ',
    'updateSkill'=>'تعديل المهارة ',
    'Levellangskil'=>'المستوى',
    'Namelangskil'=>'الاسم',
    'UserProfile'=>'الملف الشخصي للمستخدم',
    'Resumes'=>' السيرة الذاتية',
    'Letters'=>'خطاب النويا ',
    'temp1'=>'النموذج الاول ',
    'temp2'=>'النموذج الثاني',
    'temp3'=>'النموذج الثالث',
    'backgroundColor'=>'لون الخلفية ',
    'fontColor'=>'لون النص ',
    'download'=>'تحميل الملف ',
    'preview'=>'القاء نظرة ',
    'companyemail'=>'ايميل الجهة ',
    'companyName'=>'اسم الجهة',
    'companywebsite'=>'رابط الجهة ',
    'comphone'=>'هاتف الجهة ',
    'personname'=>'الشخص المسؤول',
    'date'=>'تاريخ الكتابة',
    'CoverLetters'=>'محتوى الخطاب ',
    'ExperienceTitle'=>'مسمى الوظيفة',
    'ExperienceSupTitle'=>'جهة العمل',
    'EducationTitle'=>'التخصص/الدورة',
    'EducationSupTitle'=>'الجهة التعليمية',
    'ProjectsTitle'=>'اسم المشروع',
    'ProjectsSupTitle'=>'رابط المشروع',
    'Genderf'=>'انثى',
    'Genderm'=>'ذكر',
    'changpassword'=>'تغيير كلمة السر',
    'chang'=>'تغيير ',
    ''=>'',
    ''=>'',
    ''=>'',
    ''=>'',
    ''=>'',
    ''=>'',
  ],
  'Notification'=>[
    'Notifications'=> 'التنبيهات ',
    'Messages'=> 'الرسائل ',
    'MessagesAll'=>'كافة الرسائل ',
    'NotificationsAll'=>'كافة التنبيهات ',
    'Search'=>'بحث ',
    'Notification'=> 'التنبية ',
    'Time'=>'الوقت ',
    'Type'=>'النوع',
    'SeeNew'=>'عرض التنبية ',
    'rejectJob'=>'الغاء الوطيفة',
    'rejectYes'=>'نعم, الغاء',
    'rejectTender'=>'الغاء المناقصة',
    'rejectNo'=>'لا,الغاء الامر ',
    'sureRejectJob'=>'هل انت متاكد من الغاء الوظيفة؟',
    'sureRejectTender'=>'هل انت متاكد من الغاء المناقصة؟',
    'acceptJob'=>'قبول الوظيفة ',
    'acceptTender'=>'قبول المناقصة ',
    'accept'=>'قبول النشر ',
    'sureAcceptJob'=>'هذة الوظيفة سوف نتشر في الموقع!',
    'sureAcceptTender'=>'هذة المناقصة سوف تنشر في الموقع!',
    'postTender'=>'تفاصيل المناقصة ',
    'postJob'=>'تفاصيل الوظيفة ',
    'postJobmessage'=>'هذة الوظيفة تحتاج الى معالجة ',
    'postTendermessage'=>'هذة المناقصة تحتاج الى معالجة',
    'tender'=>'مناقصة',
    'job'=>'وظيفة',
    'both'=>'المناقصات والوظائف ',
    'All'=>'كافة الاقسام',
    'crateNotify'=>'انشاء تنبيهات للمناقصات والوظائف ',
    'NotifyType'=>'نوع التنبية ',
    'explaine'=>' سوف يتم ارسال تنبيهات الى ايميلك يوميا من فضلك قم باختيار نوع التنبية والقسم الخاص به ',
    'createNotifyNow'=>'انشاء تنبية',
    'addAccountmessage'=>'هذا الحساب يحتاج الى معالجة',
    'rejectAccount'=>'الغاء الحساب ',
    'acceptAccount'=>'قبول الحساب ',
    'sureRejectAccount'=>'هل انت متاكد من الغاء الحساب ؟ ',
    'sureAcceptAccount'=>'هذا الحساب سوف يكون مشترك في الموقع !',
    'acceptA'=>'قبول الاشتراك',
    'activeAccount'=>'هذا الحساب فعال ومشترك في الموقع',
    'waitingAccount'=>'هذا الحساب قيد الانتظار ',
    'waitAccount'=>'حسابات قيد الانتظار ',
    'rejectingAccount'=>'هذا الحساب تم رفضة من ادارة الموقع',
    'waitJob'=>'وظائف قيد الانتظار ',
    'waitTender'=>'مناقصات قيد الانتظار ',
    'activeJob'=>'هذه الوظيفة تم قبولها وسوف تنشر في الوقت المحدد',
    'activeTender'=>'هذه المناقصة تم قبولها وسوف تنشر في الوقت المحدد',
    'waitingJob'=>'هذه الوظيفة قيد الانتظار ',
    'waitingTender'=>'هذه المناقصة قيد الانتظار ',
    'rejectingJob'=>'هذه الوظيفة تم رفضها من ادارة الموقع ولن يتم النشر',
    'rejectingTender'=>'هذه المناقصة تم رفضها من ادارة الموقع ولن يتم النشر',
    'deadline'=>'منتهية الصلاحية , لقد تم انتهاء الوقت المحدد ',
    'doneReject' => 'تم الالغاء',
    'doneAccpt' => ' تم القبول  ',
    'newMessage'=>"رسالة جديدة",
    'newUser'=>"اشترك جديد ",
    'newPostingJob'=>" طلب نشر وظيفة جديدة",
    'newPostingTender'=>"طلب نشر مناقصة جديدة",
    'Status'=>'حالة التنبية ',
    'see' => 'تمت مشاهدتها  ',
    'notsee' => 'لم تتم مشاهدتها ',
    'NotificationLater'=>'إغلاق وذكرني مرة اخرى',
    'NotificationsSet'=>'ضبط اشعارات الايميل ',
    'newNotifications' => 'تنبيهات جديدة '




  ],
  'cpanelhome'=>[
    'lastJobs'=> 'اخر الوظائف حسب الشركات',
    'morejobs'=>'عرض الوظائف',
    'viewAll'=>'عرض الجميع ',
    'moretenders'=>'عرض المناقصات',
    'postedby'=> 'نشر بواسطة',
    'lastTenders'=> 'اخر المناقصات حسب الشركات',
    'lastUsers'=> ' اخر المستخدمين المشتركين في الموقع',
    'lastCompanies'=> 'اخرالشركات المشتركه في الموقع',
    'Dashboard'=> 'لوحة التحكم',
    'home'=> 'الرئيسية',
  ],
  'reports'=>[
    'title'=> 'التقارير',
    'home'=> 'الرئيسية',
    'totalBlogs'=> 'عدد المقالات',
    'totalServices'=> 'عدد الخدمات',
    'tenderReport'=> 'تقرير المناقصات',
    'totalTenders'=> 'عدد المناقصات',
    'activeTenders'=> 'المناقصات المفعله',
    'inactiveTenders'=> 'المناقصات الملغية',
    'jobReport'=> 'تقرير الوظائف',
    'totalJobs'=> 'عدد الوظائف',
    'activeJobs'=> 'الوظائف المفعله',
    'inactiveJobs'=> 'الوظائف الملغية',
    'usersReport'=> 'تقرير المشتركين',
    'totalUsers'=> 'عدد المشتركين',
    'resumeDownload'=> 'السير الذاتية  ',
    'coverLetterDownload'=> 'خطاب النوايا  ',
    'resumeTemplate'=> 'قالب السيرة الذاتية الاكثر تنزيلا',
    'coverLetterTemplate'=> 'قالب الخطاب الاكثر تنزيلا',
    'advertismentReport'=> 'تقرير الاعلانات',
    'advertismentTotal'=> 'عدد الاعلانات',
    'companyReport'=> 'تقرير الشركات',
    'totalCompanies'=> 'عدد الشركات',
    'totalCompany'=> 'عدد الشركات المسجلة',
    'activeCompanies'=> 'عدد الشركات المقبولة',
    'inactiveCompanies'=> 'عدد الشركات الملغية',
    'companiesJobs'=> 'عدد الوظائف التي نشرت حسب الشركة',
    'companiesTenders'=> 'عدد المناقصات التي نشرت حسب الشركة',
    'total_visitors'=>'عدد الزوار'
  ]
];
